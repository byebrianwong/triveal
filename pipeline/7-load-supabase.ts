...
for (const q of questions) {
  ...
  const { error: clueErr } = await sb.from("triveal_clues").insert(
    q.clues.map((c) => ({
      question_id: row.id,
      position: c.position,
      text: c.text,
  ...
  const { error: decoyErr } = await sb.from("triveal_decoys").insert(
    q.decoys.map((d) => ({
      question_id: row.id,
      text: d.text,
  ...
  if (clueErr || decoyErr) {
    ...
    await sb.from("triveal_questions").delete().eq("id", row.id); // keep the bank consistent
    continue;
  }
  inserted++;
